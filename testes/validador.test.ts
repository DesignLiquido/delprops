...
function criarEsquemas(): Map<string, DefinicaoPropriedadeInterface[]> {
  ...
  esquemas.set("liquido.dados", [
    {
      ...
      tipo: "texto",
      detalhe: "Tecnologia.",
      valoresPermitidos: ["sqlite", "postgres"],
    },
...
describe("Módulo Validador", () => {
  ...
  describe("validar", () => {
    let esquemas: Map<string, DefinicaoPropriedadeInterface[]>;
    ...
    it("deve validar liquido.dados com sub-namespace dinâmico", () => {
      const props: PropriedadeCompreendidaInterface[] = [
        {
          chave: "liquido.dados.lincones.tecnologia",
          valor: "'sqlite'",
          linha: 1,
        },
      ];
    ...
    it("deve gerar erro para valor não permitido", () => {
      ...
      expect(resultado.avisos).toEqual([]);
      expect(resultado.erros).toHaveLength(1);
      expect(resultado.erros[0].mensagem).toContain(
        "não está entre os permitidos",
      );
    });
    ...
    it("deve gerar erro para valor sem tipo conhecido", () => {
      ...
      expect(resultado.erros).toHaveLength(1);
      expect(resultado.erros[0].mensagem).toContain(
        "não corresponde a nenhum tipo conhecido",
      );
    });
